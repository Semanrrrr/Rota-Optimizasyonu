...
class MetroAgi:
    ...
    def en_hizli_rota_bul(self, baslangic_id: str, hedef_id: str) -> Optional[Tuple[List[Istasyon], int]]:
        ...
        while pq:
            toplam_sure, istasyon, rota = heapq.heappop(pq)
...
if __name__ == "__main__":
    ...
    print("2. Batıkent'ten Keçiören'e:")
    ...
    if sonuc:
     ...
     # Senaryo 4: Kızılay'dan Demetevler'e      
    print("4. Kızılay'dan Demetevler'e :")
    rota = metro.en_az_aktarma_bul("K1", "T2")
    if rota:
         print("En az aktarmalı rota:", " -> ".join(i.ad for i in rota))
     
    sonuc = metro.en_hizli_rota_bul("K1", "T2")
    if sonuc:
         rota, sure = sonuc
         print(f"En hızlı rota ({sure} dakika):", " -> ".join(i.ad for i in rota)) 
        
    # Senaryo 5: Ulus'tan AŞTİ'ye      
    print("5. Ulus'tan AŞTİ'ye   :")
    rota = metro.en_az_aktarma_bul("K2", "M1")
    if rota:
        print("En az aktarmalı rota:", " -> ".join(i.ad for i in rota))
    
    sonuc = metro.en_hizli_rota_bul("K2", "M2")
    if sonuc:
        rota, sure = sonuc
        print(f"En hızlı rota ({sure} dakika):", " -> ".join(i.ad for i in rota))      
